...
int copy_file3(const char *src, const char *dst) {

    if (!src || !dst) {
        return CP_ILLEGAL_ARGS;
    }

    FILE *fp_src = fopen(src, "rb"); // 在 windows 上拷贝非文本文件，需要加上 b
    if (!fp_src) {
        return CP_SRC_OPEN_ERR;
    }

    FILE *fp_dst = fopen(dst, "wb"); // 在 windows 上拷贝非文本文件，需要加上 b
    if (!fp_dst) {
        fclose(fp_src);
        return CP_DST_OPEN_ERR;
    }
    ...
    char buffer[BUFFER_SIZE];
    while (1) {
        size_t bytes_read = fread(buffer, sizeof(buffer[0]), BUFFER_SIZE, fp_src);

        if (fwrite(buffer, sizeof(buffer[0]), BUFFER_SIZE, fp_dst) < bytes_read) {
            result = CP_DST_WRITE_ERR;
            break;
        }
        ...
        if (bytes_read < BUFFER_SIZE) {
            if (ferror(fp_src)) {
                ...
                fwrite(buffer, sizeof(buffer[0]), bytes_read, fp_dst);
                result = CP_OK;
            } else {
                result = CP_UNKNOWN_ERR;
            }
...
//    setlocale(LC_ALL, "zh_CN.utf-8"); // 中文文件名称
    ...
    run(copy_file3, "static/harry_potter.txt", "static/harry_potter_3.txt");
